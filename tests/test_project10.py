...
@generate_exercises(40, 41)
class Project10(Project) :
    ...
    def test_3_not_exists(self) :
        ...
        self.assertIsNotNone(filename, "I can't find your project file (project10.py)")
        article = 'aabaeasdf'
        with open(f'logs/test_3_check_bogus.out', 'a') as log :
    ...
    def do_check(self, article, words) :
        ...
        words = [ f'(?i){word}' for word in set(words) ]
        filename = self.find_file('project10.py')
        self.assertIsNotNone(filename, "I can't find your project file (project10.py)")

        with open(f'logs/test_check_{article}.out', 'a') as log :
            test = pexpect.spawnu('python "' + filename.as_posix() + f'" "{article}"', logfile=log, encoding='utf-8')
            for word in words : 
